...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
    ...
    if(!fs::exists(GUIUtil::QStringToPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/opensy"));
        did_show_intro = true;
     ...
     * override -datadir in the opensy.conf file in the default data directory
     * (to be consistent with opensyd behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    static constexpr uint64_t nPowTargetSpacing = 2 * 60;  // OpenSY uses 2-minute blocks
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the OpenSY block chain.").arg(CLIENT_NAME) + " " +
