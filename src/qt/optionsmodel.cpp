...
/** Call node.updateRwSetting() with OpenSY 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because OpenSY 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/opensyria/OpenSY/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because opensy 22.x will not try to read these.
...
//! Convert enabled/size values to opensy -prune setting.
...
//! Get pruning enabled value to show in GUI from opensy -prune setting.
...
//! Get pruning size value to show in GUI from opensy -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayOpenSYUnit")) {
        settings.setValue("DisplayOpenSYUnit", QVariant::fromValue(OpenSYUnit::SYL));
    }
    QVariant unit = settings.value("DisplayOpenSYUnit");
    if (unit.canConvert<OpenSYUnit>()) {
        m_display_opensy_unit = unit.value<OpenSYUnit>();
    } else {
        m_display_opensy_unit = OpenSYUnit::SYL;
        settings.setValue("DisplayOpenSYUnit", QVariant::fromValue(m_display_opensy_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if opensy.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_opensy_unit);
...
{
    if (new_unit.isNull() || new_unit.value<OpenSYUnit>() == m_display_opensy_unit) return;
    m_display_opensy_unit = new_unit.value<OpenSYUnit>();
    QSettings settings;
    settings.setValue("DisplayOpenSYUnit", QVariant::fromValue(m_display_opensy_unit));
    Q_EMIT displayUnitChanged(m_display_opensy_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/opensyria/OpenSY/pull/8273
    ...
    // (https://github.com/opensy-core/gui/issues/567).
