This directory contains the source code for the OpenSY graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the OpenSY UI.
...
- ClientModel is used by the main application `opensygui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following an opensy:... link from a browser).
...
* UI elements like OpenSYAmountField, which inherit from QWidget.
* `opensystrings.cpp`: automatically generated
* `opensyunits.(h/cpp)`: SYL / mSYL / etc. handling
* `callback.h`
...
**Note:** Do not change `local/opensy_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `opensy-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `opensy-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `opensy-qt`
9. You're all set! Start developing, building, and debugging the OpenSY GUI
