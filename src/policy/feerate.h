...
/**
 * Fee rate in qirsh per virtualbyte: CAmount / vB
...
public:
    ...
    /**
     * Construct a fee rate from a fee in qirsh and a vsize in vB.
     *
    ...
    /**
     * Return the fee in qirsh for the given vsize in vbytes.
     * If the calculated fee would have fractional qirsh, then the
     * returned fee will always be rounded up to the nearest qirsh.
     */
    CAmount GetFee(int32_t virtual_bytes) const;

    /**
     * Return the fee in qirsh for a vsize of 1000 vbytes
     */
    ...
    std::string ToString(const FeeEstimateMode& fee_estimate_mode = FeeEstimateMode::SYL_KVB) const;
