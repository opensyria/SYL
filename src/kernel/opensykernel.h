 ...
 */
typedef struct osck_Transaction osck_Transaction;

/**
 * Opaque data structure for holding a script pubkey.
 */
typedef struct osck_ScriptPubkey osck_ScriptPubkey;

/**
 * Opaque data structure for holding a transaction output.
 */
typedef struct osck_TransactionOutput osck_TransactionOutput;
 ...
 * @ref osck_logging_disable. Functions changing the logging settings are
 * global and change the settings for all existing osck_LoggingConnection
 * instances.
 */
typedef struct osck_LoggingConnection osck_LoggingConnection;
 ...
 */
typedef struct osck_ChainParameters osck_ChainParameters;
 ...
 * time.
 */
typedef struct osck_ContextOptions osck_ContextOptions;
 ...
 */
typedef struct osck_Context osck_Context;
 ...
 */
typedef struct osck_BlockTreeEntry osck_BlockTreeEntry;
 ...
 * chainstate manager.
 */
typedef struct osck_ChainstateManagerOptions osck_ChainstateManagerOptions;
 ...
 */
typedef struct osck_ChainstateManager osck_ChainstateManager;

/**
 * Opaque data structure for holding a block.
 */
typedef struct osck_Block osck_Block;
 ...
 */
typedef struct osck_BlockValidationState osck_BlockValidationState;

/**
 * Opaque data structure for holding the currently known best-chain associated
 * with a chainstate.
 */
typedef struct osck_Chain osck_Chain;
 ...
 * @ref osck_TransactionSpentOutputs. Each osck_TransactionSpentOutputs is in
 * turn a vector of all the previous outputs of a transaction (in order of
 * their corresponding inputs).
 */
typedef struct osck_BlockSpentOutputs osck_BlockSpentOutputs;
 ...
 * @ref osck_BlockSpentOutputs. The coins are in the same order as the
 * transaction's inputs consuming them.
 */
typedef struct osck_TransactionSpentOutputs osck_TransactionSpentOutputs;

/**
 * Opaque data structure for holding a coin.
 *
 * Holds information on the @ref osck_TransactionOutput held within,
 * including the height it was spent at and whether it is a coinbase output.
 */
typedef struct osck_Coin osck_Coin;
 ...
 */
typedef struct osck_BlockHash osck_BlockHash;

/**
 * Opaque data structure for holding a transaction input.
 *
 * Holds information on the @ref osck_TransactionOutPoint held within.
 */
typedef struct osck_TransactionInput osck_TransactionInput;
 ...
 */
typedef struct osck_TransactionOutPoint osck_TransactionOutPoint;

typedef struct osck_Txid osck_Txid;

/** Current sync state passed to tip changed callbacks. */
typedef uint8_t osck_SynchronizationState;
#define osck_SynchronizationState_INIT_REINDEX ((osck_SynchronizationState)(0))
#define osck_SynchronizationState_INIT_DOWNLOAD ((osck_SynchronizationState)(1))
#define osck_SynchronizationState_POST_INIT ((osck_SynchronizationState)(2))

/** Possible warning types issued by validation. */
typedef uint8_t osck_Warning;
#define osck_Warning_UNKNOWN_NEW_RULES_ACTIVATED ((osck_Warning)(0))
#define osck_Warning_LARGE_WORK_INVALID_CHAIN ((osck_Warning)(1))
 ...
 */
typedef void (*osck_DestroyCallback)(void* user_data);

/**
 * Function signatures for the kernel notifications.
 */
typedef void (*osck_NotifyBlockTip)(void* user_data, osck_SynchronizationState state, const osck_BlockTreeEntry* entry, double verification_progress);
typedef void (*osck_NotifyHeaderTip)(void* user_data, osck_SynchronizationState state, int64_t height, int64_t timestamp, int presync);
typedef void (*osck_NotifyProgress)(void* user_data, const char* title, size_t title_len, int progress_percent, int resume_possible);
typedef void (*osck_NotifyWarningSet)(void* user_data, osck_Warning warning, const char* message, size_t message_len);
typedef void (*osck_NotifyWarningUnset)(void* user_data, osck_Warning warning);
typedef void (*osck_NotifyFlushError)(void* user_data, const char* message, size_t message_len);
typedef void (*osck_NotifyFatalError)(void* user_data, const char* message, size_t message_len);

/**
 * Function signatures for the validation interface.
 */
typedef void (*osck_ValidationInterfaceBlockChecked)(void* user_data, osck_Block* block, const osck_BlockValidationState* state);
typedef void (*osck_ValidationInterfacePoWValidBlock)(void* user_data, osck_Block* block, const osck_BlockTreeEntry* entry);
typedef void (*osck_ValidationInterfaceBlockConnected)(void* user_data, osck_Block* block, const osck_BlockTreeEntry* entry);
typedef void (*osck_ValidationInterfaceBlockDisconnected)(void* user_data, osck_Block* block, const osck_BlockTreeEntry* entry);

/**
 * Function signature for serializing data.
 */
typedef int (*osck_WriteBytes)(const void* bytes, size_t size, void* userdata);

/**
 * Whether a validated data structure is valid, invalid, or an error was
 * encountered during processing.
 */
typedef uint8_t osck_ValidationMode;
#define osck_ValidationMode_VALID ((osck_ValidationMode)(0))
#define osck_ValidationMode_INVALID ((osck_ValidationMode)(1))
#define osck_ValidationMode_INTERNAL_ERROR ((osck_ValidationMode)(2))

/**
 * A granular "reason" why a block was invalid.
 */
typedef uint32_t osck_BlockValidationResult;
#define osck_BlockValidationResult_UNSET ((osck_BlockValidationResult)(0))           //!< initial value. Block has not yet been rejected
#define osck_BlockValidationResult_CONSENSUS ((osck_BlockValidationResult)(1))       //!< invalid by consensus rules (excluding any below reasons)
#define osck_BlockValidationResult_CACHED_INVALID ((osck_BlockValidationResult)(2))  //!< this block was cached as being invalid and we didn't store the reason why
#define osck_BlockValidationResult_INVALID_HEADER ((osck_BlockValidationResult)(3))  //!< invalid proof of work or time too old
#define osck_BlockValidationResult_MUTATED ((osck_BlockValidationResult)(4))         //!< the block's data didn't match the data committed to by the PoW
#define osck_BlockValidationResult_MISSING_PREV ((osck_BlockValidationResult)(5))    //!< We don't have the previous block the checked one is built on
...
typedef struct {
    ...
    osck_DestroyCallback user_data_destroy;                       //!< Frees the provided user data structure.
    osck_ValidationInterfaceBlockChecked block_checked;           //!< Called when a new block has been fully validated. Contains the
                                                                  //!< result of its validation.
    osck_ValidationInterfacePoWValidBlock pow_valid_block;        //!< Called when a new block extends the header chain and has a valid transaction
                                                                  //!< and segwit merkle root.
    osck_ValidationInterfaceBlockConnected block_connected;       //!< Called when a block is valid and has now been connected to the best chain.
    osck_ValidationInterfaceBlockDisconnected block_disconnected; //!< Called during a re-org when a block has been removed from the best chain.
...
typedef struct {
    ...
    osck_DestroyCallback user_data_destroy; //!< Frees the provided user data structure.
    osck_NotifyBlockTip block_tip;          //!< The chain's tip was updated to the provided block entry.
    osck_NotifyHeaderTip header_tip;        //!< A new best block header was added.
    osck_NotifyProgress progress;           //!< Reports on current block synchronization progress.
    osck_NotifyWarningSet warning_set;      //!< A warning issued by the kernel library during validation.
    osck_NotifyWarningUnset warning_unset;  //!< A previous condition leading to the issuance of a warning is no longer given.
    osck_NotifyFlushError flush_error;      //!< An error encountered when flushing data to disk.
    osck_NotifyFatalError fatal_error;      //!< An unrecoverable system error encountered by the library.
} osck_NotificationInterfaceCallbacks;

/**
 * A collection of logging categories that may be encountered by kernel code.
 */
typedef uint8_t osck_LogCategory;
#define osck_LogCategory_ALL ((osck_LogCategory)(0))
#define osck_LogCategory_BENCH ((osck_LogCategory)(1))
#define osck_LogCategory_BLOCKSTORAGE ((osck_LogCategory)(2))
#define osck_LogCategory_COINDB ((osck_LogCategory)(3))
#define osck_LogCategory_LEVELDB ((osck_LogCategory)(4))
#define osck_LogCategory_MEMPOOL ((osck_LogCategory)(5))
#define osck_LogCategory_PRUNE ((osck_LogCategory)(6))
#define osck_LogCategory_RAND ((osck_LogCategory)(7))
#define osck_LogCategory_REINDEX ((osck_LogCategory)(8))
#define osck_LogCategory_VALIDATION ((osck_LogCategory)(9))
#define osck_LogCategory_KERNEL ((osck_LogCategory)(10))

/**
 * The level at which logs should be produced.
 */
typedef uint8_t osck_LogLevel;
#define osck_LogLevel_TRACE ((osck_LogLevel)(0))
#define osck_LogLevel_DEBUG ((osck_LogLevel)(1))
#define osck_LogLevel_INFO ((osck_LogLevel)(2))
 ...
 */
typedef uint8_t osck_ScriptVerifyStatus;
#define osck_ScriptVerifyStatus_OK ((osck_ScriptVerifyStatus)(0))
#define osck_ScriptVerifyStatus_ERROR_INVALID_FLAGS_COMBINATION ((osck_ScriptVerifyStatus)(1)) //!< The flags were combined in an invalid way.
#define osck_ScriptVerifyStatus_ERROR_SPENT_OUTPUTS_REQUIRED ((osck_ScriptVerifyStatus)(2))    //!< The taproot flag was set, so valid spent_outputs have to be provided.

/**
 * Script verification flags that may be composed with each other.
 */
typedef uint32_t osck_ScriptVerificationFlags;
#define osck_ScriptVerificationFlags_NONE ((osck_ScriptVerificationFlags)(0))
#define osck_ScriptVerificationFlags_P2SH ((osck_ScriptVerificationFlags)(1U << 0))                 //!< evaluate P2SH (BIP16) subscripts
#define osck_ScriptVerificationFlags_DERSIG ((osck_ScriptVerificationFlags)(1U << 2))               //!< enforce strict DER (BIP66) compliance
#define osck_ScriptVerificationFlags_NULLDUMMY ((osck_ScriptVerificationFlags)(1U << 4))            //!< enforce NULLDUMMY (BIP147)
#define osck_ScriptVerificationFlags_CHECKLOCKTIMEVERIFY ((osck_ScriptVerificationFlags)(1U << 9))  //!< enable CHECKLOCKTIMEVERIFY (BIP65)
#define osck_ScriptVerificationFlags_CHECKSEQUENCEVERIFY ((osck_ScriptVerificationFlags)(1U << 10)) //!< enable CHECKSEQUENCEVERIFY (BIP112)
#define osck_ScriptVerificationFlags_WITNESS ((osck_ScriptVerificationFlags)(1U << 11))             //!< enable WITNESS (BIP141)
#define osck_ScriptVerificationFlags_TAPROOT ((osck_ScriptVerificationFlags)(1U << 17))             //!< enable TAPROOT (BIPs 341 & 342)
#define osck_ScriptVerificationFlags_ALL ((osck_ScriptVerificationFlags)(osck_ScriptVerificationFlags_P2SH |                \
                                                                         osck_ScriptVerificationFlags_DERSIG |              \
                                                                         osck_ScriptVerificationFlags_NULLDUMMY |           \
                                                                         osck_ScriptVerificationFlags_CHECKLOCKTIMEVERIFY | \
                                                                         osck_ScriptVerificationFlags_CHECKSEQUENCEVERIFY | \
                                                                         osck_ScriptVerificationFlags_WITNESS |             \
                                                                         osck_ScriptVerificationFlags_TAPROOT))

typedef uint8_t osck_ChainType;
#define osck_ChainType_MAINNET ((osck_ChainType)(0))
#define osck_ChainType_TESTNET ((osck_ChainType)(1))
#define osck_ChainType_TESTNET_4 ((osck_ChainType)(2))
#define osck_ChainType_SIGNET ((osck_ChainType)(3))
#define osck_ChainType_REGTEST ((osck_ChainType)(4))
 ...
 * `osck_ScriptVerificationFlags_WITNESS` flag is set in the flags bitfield, the
 ...
 * @param[in] flags             Bitfield of osck_ScriptVerificationFlags controlling validation constraints.
 ...
 * osck_LoggingConnection instances.
 *
 ...
 * enable the selected categories. Use @ref osck_logging_enable_category to
 * start logging from a specific, or all categories. This changes a global
 * setting and will override settings for all existing
 * @ref osck_LoggingConnection instances.
 *
 * @param[in] category If osck_LogCategory_ALL is chosen, sets both the global fallback log level
 *                     used by all categories that don't have a specific level set, and also
 *                     sets the log level for messages logged with the osck_LogCategory_ALL category itself.
 ...
 *
 * @param[in] context_options                Non-null, previously created with osck_context_options_create.
 ...
 * @ref osck_chainstate_manager_import_blocks this triggers either a full reindex,
