...
to a hash that has been compiled into opensyd.
...
class AssumeutxoTest(OpenSYTestFramework):
    ...
    def test_invalid_snapshot_scenarios(self, valid_snapshot_path):
        ...
        invalid_magics = [
            # magic, name, real (recognized by OpenSY)
            [MAGIC_BYTES["mainnet"], "main", True],
            [MAGIC_BYTES["testnet4"], "testnet4", True],
            # Note: signet uses Bitcoin magic bytes which OpenSY doesn't recognize
            [MAGIC_BYTES["signet"], "signet", False],
        ...
        cases = [
            ...
            # These corruptions should cause either hash mismatch or bad snapshot data errors
            [b"\xff" * 32, 0, "Bad snapshot"],  # wrong outpoint hash
            [(2).to_bytes(1, "little"), 32, "Bad snapshot"],  # wrong txid coins count
            [b"\xfd\xff\xff", 32, "Mismatch in coins count"],  # txid coins count exceeds coins left
            [b"\x01", 33, "Bad snapshot"],  # wrong outpoint index
            [b"\x82", 34, "Bad snapshot"],  # wrong coin code VARINT
            [b"\x80", 34, "Bad snapshot"],  # another wrong coin code
            [b"\x84\x58", 34, "Bad snapshot"],  # wrong coin case
            [
    ...
    def test_invalid_mempool_state(self, dump_output_path):
        self.log.info("Test opensyd should fail when mempool not empty.")
        node=self.nodes[2]
    ...
    def test_invalid_file_path(self):
        self.log.info("Test opensyd should fail when file path is invalid.")
        node = self.nodes[0]
    ...
    def test_snapshot_with_less_work(self, dump_output_path):
        self.log.info("Test opensyd should fail when snapshot has less accumulated work than this node.")
        node = self.nodes[0]
    ...
    def run_test(self):
        ...
        raw_tx = n1.createrawtransaction([prevout], {getnewdestination()[2]: 4999.99})
