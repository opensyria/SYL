...
class WalletTest(OpenSYTestFramework):
    ...
    def run_test(self):
        ...
        # Send 8000 SYL from 0 to 1 and 12000 SYL from 1 to 0.
        txs = create_transactions(self.nodes[0], self.nodes[1].getnewaddress(), 8000, [Decimal('0.01')])
        self.nodes[0].sendrawtransaction(txs[0]['hex'])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])  # sending on both nodes is faster than waiting for propagation

        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), 12000, [Decimal('0.01'), Decimal('0.02')])
        ...
        # Before `test_balance()`, we have had two nodes with a balance of 10000
        # each and then we:
        #
        # 1) Sent 8000 from node A to node B with fee 0.01
        # 2) Sent 12000 from node B to node A with fee 0.01
        #
        ...
        #
        # 1) Sent 8000 from node A to node B with fee 0.01
        # 2) Sent 2000 from node B to node A with fee 0.01
        #
        # Then our node would report a confirmed balance of 8000 + 10000 - 2000 = 16000
        # SYL, which is more than would be available if transaction 1 were
        # replaced.
        ...
        def test_balances(*, fee_node_1=0):
            ...
            expected_balances_0 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('1999.99'),  # change from node 0's send
                                                 'untrusted_pending': Decimal('12000.0')}}
            expected_balances_1 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('0E-8'),  # node 1's send had an unsafe input
                                                 'untrusted_pending': Decimal('6000.0') - fee_node_1}}  # Doesn't include output of node 0's send since it was spent
            ...
            assert_equal(self.nodes[0].getbalance(), Decimal('1999.99'))  # change from node 0's send
            assert_equal(self.nodes[1].getbalance(), Decimal('0'))  # node 1's send had an unsafe input
            # Same with minconf=0
            assert_equal(self.nodes[0].getbalance(minconf=0), Decimal('1999.99'))
        ...
        balance_node0 = Decimal('13999.99')  # node 1's send plus change from node 0's send
        balance_node1 = Decimal('5999.98')  # change from node 0's send
        ...
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), Decimal('5999.97'), [Decimal('0.01')])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])
        ...
        for _ in range(3):
            ...
            txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), 19800)
        ...
        tx_replace = tx_orig.replace(
            (19800 * 10**8).to_bytes(8, "little", signed=True).hex(),
            (19799 * 10**8).to_bytes(8, "little", signed=True).hex(),
        )
