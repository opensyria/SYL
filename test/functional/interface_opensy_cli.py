...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"opensy-cli -h\" for help or \"opensy-cli listwallets\" to see "
...
class TestOpenSYCli(OpenSYTestFramework):
    ...
    def test_netinfo(self):
        ...
        # Service flags by bit position: n(0) w(3) l(10) 2(11) r(24=RANDOMX, always set)
        assert re.match(rf"^{re.escape(self.config['environment']['CLIENT_NAME'])} client.+services nwl?2?r$", det[0])
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `opensy-cli getblockchaininfo`")
        ...
        # Prevent opensy-cli from using existing rpcport in conf
        ...
        self.log.info("Test -getinfo and opensy-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and opensy-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (SYL/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (SYL/kvB)']), network_info['relayfee'])
            ...
            # Test opensy-cli -generate with -rpcwallet in multiwallet mode.
