...
class AcceptBlockTest(OpenSYTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].getblock(all_blocks[1438].hash_hex)
        assert_equal(self.nodes[0].getbestblockhash(), all_blocks[1438].hash_hex)
        assert_raises_rpc_error(-1, "Block not available (not fully downloaded)", self.nodes[0].getblock, all_blocks[1439].hash_hex)
        self.log.info("Successfully reorged to longer chain")

        # 8. Create a chain which is invalid at a height longer than the
        # current chain, but which has more blocks on top of that
        block_1441f = create_block(all_blocks[1436].hash_int, create_coinbase(1441), all_blocks[1436].nTime+1)
        block_1441f.solve()
        block_1442f = create_block(block_1441f.hash_int, create_coinbase(1442), block_1441f.nTime+1)
        block_1442f.solve()
        # block_1443 spends a coinbase below maturity!
        tx_to_add = create_tx_with_script(block_1442f.vtx[0], 0, script_sig=b"42", amount=1)
        block_1443 = create_block(block_1442f.hash_int, create_coinbase(1443), block_1442f.nTime+1, txlist=[tx_to_add])
        block_1443.solve()
        block_1444 = create_block(block_1443.hash_int, create_coinbase(1444), block_1443.nTime+1)
        block_1444.solve()
        ...
        tip_entry_found = False
        for x in self.nodes[0].getchaintips():
            if x['hash'] == block_1444.hash_hex:
                assert_equal(x['status'], "headers-only")
                tip_entry_found = True
        assert tip_entry_found
        assert_raises_rpc_error(-1, "Block not available (not fully downloaded)", self.nodes[0].getblock, block_1444.hash_hex)

        test_node.send_without_ping(msg_block(block_1441f))
        test_node.send_and_ping(msg_block(block_1442f))

        self.nodes[0].getblock(block_1441f.hash_hex)
        self.nodes[0].getblock(block_1442f.hash_hex)

        test_node.send_without_ping(msg_block(block_1443))
        ...
        assert_equal(self.nodes[0].getblockcount(), 1442)
        assert_equal(self.nodes[0].getbestblockhash(), all_blocks[1438].hash_hex)
        assert_equal(self.nodes[0].getblock(block_1443.hash_hex)["confirmations"], -1)

        # Now send a new header on the invalid chain, indicating we're forked off, and expect to get disconnected
        block_1445 = create_block(block_1444.hash_int, create_coinbase(1445), block_1444.nTime+1)
        block_1445.solve()
        headers_message = msg_headers()
