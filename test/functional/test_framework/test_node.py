...
"""Class for opensyd node under test"""
...
# 'opensy' command, it may internally insert more args, which must be
...
OPENSY_PID_FILENAME_DEFAULT = "opensyd.pid"
...
class TestNode():
    """A class for representing a opensyd node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, binaries, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, v2transport=False, uses_wallet=False, ipcbind=False):
        ...
        self.opensyconf = self.datadir_path / "opensy.conf"
        ...
        # Configuration for logging is set as command-line args rather than in the opensy.conf file.
        # This means that starting a opensyd using the temp dir to debug a failed test won't
        # spam debug.log.
    ...
    PRIV_KEYS = [
            # address , privkey (testnet/regtest addresses start with 'f' for Freedom)
            AddressKeyPair('fJ46kJT8L69bf86DKiamosUscWRNe6Xzoc', 'cVpF924EspNh8KjYsfhgY96mmxvT6DgdWiTYMtMjuM74hJaU5psW'),
            AddressKeyPair('fS7SyfmKbpxgs7AhdfzYaPW7nmSKHRHaRh', 'cUxsWyKyZ9MAQTaAhUQWJmBbSvHMwSmuv59KgxQV7oZQU3PXN3KE'),
            AddressKeyPair('fMQ8SdCfnaM9nWVexMp87avYD1vNjUPPuc', 'cTrh7dkEAeJd6b3MRX9bZK8eRmNqVCMH3LSUkE3dSFDyzjU38QxK'),
            AddressKeyPair('fPuG4r6WrzctZwf1qD9vcGgqQ4nUEDLgyr', 'cVuKKa7gbehEQvVq717hYcbE9Dqmq7KEBKqWgWrYBa2CKKrhtRim'),
            AddressKeyPair('fS8vrNfKGCZvgAarP3QtChEc9FiRZdfNNE', 'cQDCBuKcjanpXDpCqacNSjYfxeQj8G6CAtH1Dsk3cXyqLNC4RPuh'),
            AddressKeyPair('fbT99k2Kyo7paDXMe3Qpjkmq17SDh7E4ra', 'cQakmfPSLSqKHyMFGwAqKHgWUiofJCagVGhiB4KCainaeCSxeyYq'),
            AddressKeyPair('fYbFeDfLWmduqLkrXH4ZZ924xMYrBAJFYX', 'cQMpDLJwA8DBe9NcQbdoSb1BhmFxVjWD5gRyrLZCtpuF9Zi3a9RK'),
            AddressKeyPair('fUNHhqbHF4pNPwiNwTrGHuzZjCJnrhMWvc', 'cSXmRKXVcoouhNNVpcNKFfxsTsToY5pvB9DVsFksF1ENunTzRKsy'),
            AddressKeyPair('fP5TpXg5zrtBUkN7iXmcogWLqKAeFsd5Ky', 'cSoXt6tm3pqy43UMabY6eUTmR3eSUYFtB2iNQDGgb3VUnRsQys2k'),
            AddressKeyPair('fPf1Rds1PPUuT6YLLupV16rqoXJ5cdZCPR', 'cN55daf1HotwBAgAKWVgDcoppmUNDtQSfb7XLutTLeAgVc3u8hik'),
            AddressKeyPair('fNqWXUxukSSdbQpU76ZXmz4BgXpoGpHhJD', 'cT7qK7g1wkYEMvKowd2ZrX1E5f6JQ7TM246UfqbCiyF7kZhorpX3'),
            AddressKeyPair('fZ1zNfnkJPJ43Pk7pAJvtxbeBnqwRG4CSy', 'cPiRWE8KMjTRxH1MWkPerhfoHFn5iHPWVK5aPqjW8NxmdwenFinJ'),
    ]
    ...
    def __del__(self):
        # Ensure that we don't leave any opensyd processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then opensyd would bind P2P ports on
        ...
        # opensy.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time opensyd is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by opensyd, and
        ...
        self.running = True
        self.log.debug("opensyd started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the opensyd process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'opensyd exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; opensyd may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to opensyd after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; opensyd is still starting
    ...
    def wait_until_stopped(self, *, timeout=OPENSYD_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into opensyd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to opensyd
        expected_msg: regex that stderr should match when opensyd fails

        Will throw if opensyd starts without an error.
        Will throw if an expected_msg is provided and it does not match opensyd's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'opensyd exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'opensyd should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to opensy-cli for an individual node"""
    def __init__(self, binaries, datadir, rpc_timeout):
        ...
        self.input = None
        self.log = logging.getLogger('TestFramework.opensycli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with opensy-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run opensy-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running opensy-cli {}".format(p_args[2:]))
