...
class InitTest(OpenSYTestFramework):
    """
    ...
    def init_stress_test(self):
        ...
        def sigterm_node():
            if platform.system() == 'Windows':
                ...
                # opensyd to perform any shutdown logic.
    ...
    def init_pid_test(self):
        OPENSY_PID_FILENAME_CUSTOM = "my_fancy_opensy_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = OPENSY_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / OPENSY_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / OPENSY_PID_FILENAME_CUSTOM
