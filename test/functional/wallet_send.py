...
class WalletSendTest(OpenSYTestFramework):
    def set_test_params(self):
        ...
        getcontext().prec = 8 # Qirsh precision for Decimal
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-8, "Use fee_rate (qirsh/vB) instead of feeRate", w0.send, {w1.getnewaddress(): 1}, 6, "conservative", 1, {"feeRate": 0.01})
        ...
        for target, mode in product([-1, 0], ["syl/kb", "sat/b"]):
        ...
        # Test setting explicit fee rate just below the minimum of 1 qirsh/vB.
        self.log.info("Explicit fee rate raises RPC error 'fee rate too low' if fee_rate of 0.99999999 is passed")
        msg = "Fee rate (0.999 qirsh/vB) is lower than the minimum fee rate setting (1.000 qirsh/vB)"
        ...
        msg = "Fee rate (0.000 qirsh/vB) is lower than the minimum fee rate setting (1.000 qirsh/vB)"
        ...
        self.test_send(from_wallet=w0, to_wallet=w1, amount=10001, inputs=[utxo1],
                       expect_error=(-4, ERR_NOT_ENOUGH_PRESET_INPUTS))
        self.test_send(from_wallet=w0, to_wallet=w1, amount=10001, inputs=[utxo1], add_inputs=False,
                       expect_error=(-4, ERR_NOT_ENOUGH_PRESET_INPUTS))
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=10001, inputs=[utxo1], add_inputs=True, add_to_wallet=False)
        assert res["complete"]
