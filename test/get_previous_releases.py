...
SHA256_SUMS = {
    # OpenSY release checksums will be added here as new versions are released.
    # Format: "sha256_hash": {"tag": "vX.Y.Z", "archive": "opensy-X.Y.Z-platform.tar.gz"},
    #
    # Example (uncomment and update when v0.2.0 is released):
    # "abc123...": {"tag": "v0.2.0", "archive": "opensy-0.2.0-x86_64-linux-gnu.tar.gz"},
    # "def456...": {"tag": "v0.2.0", "archive": "opensy-0.2.0-arm64-apple-darwin.tar.gz"},
    # "789ghi...": {"tag": "v0.2.0", "archive": "opensy-0.2.0-x86_64-apple-darwin.tar.gz"},
    # "jkl012...": {"tag": "v0.2.0", "archive": "opensy-0.2.0-win64.zip"},
}
...
def download_binary(tag, args) -> int:
    ...
    # OpenSY release download path
    # TODO: Update this URL when OpenSY has a release server
    bin_path = 'releases/download/{}'.format(tag)
    ...
    archive = f'opensy-{tag[1:]}-{host}.{archive_format}'
    # TODO: Update to actual OpenSY release URL (e.g., GitHub releases)
    archive_url = f'https://github.com/opensyria/SYL/{bin_path}/{archive}'
    ...
    if host == 'win64':
        ...
        ret = subprocess.run([args.tar, '-zxf', archive, '-C', tag,
                              '--strip-components=1',
                              'opensy-{tag}'.format(tag=tag[1:])]).returncode
        if ret != 0:
    ...
    # Self-sign arm64 binaries on macOS if needed
    if args.host == "arm64-apple-darwin":
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tar', action='store',
                        help='tar command to use.', default='tar')
